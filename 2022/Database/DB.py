...
class Database(object):
    ...
    class DBIterator(object):
        ...
        def __iter__(self):
            ...
            return (self.getter(entry) for entry in self.db.entries
                    if self.selection(self.getter(entry)))
    ...
    class DBNonNullIterator(DBIterator):
        ...
        def __init__(self, db, attr, null='', selection=None):
            ...
            self.null = null
            if selection:
                self.selection = lambda entry: entry != null and \
                    selection(entry)
            else:
    ...
    def mean(self, attr, null=''):
        ...
        return self._stat(attr,
                          (lambda self, attr, null: statistics.mean(
                              self.non_null_iterator(attr, null))),
                          null)
    ...
    def median(self, attr, null=''):
        ...
        non-null entries.'''
        return self._stat(attr,
                          (lambda self, attr, null: statistics.median(
                              self.non_null_iterator(attr, null))),
                          null)
    ...
    def sort(self, attr):
        ...
        if isinstance(attr, str):
            def getter(entry): return getattr(entry, attr)
        else:
            getter = attr
